...
func (uc *UsersUseCasesImpl) Create(ctx context.Context, input repository.CreateUserDTO) (repository.User, error) {
	...
	if err == nil && len(userAlreadyExists) > 0 {
		return repository.User{}, util.NewHTTPError(http.StatusConflict, "user with this username already exists")
	}

	userAlreadyExists, err = uc.List(querybuilder.WithBuilder(ctx, querybuilder.New().And("email", "eq", input.Email)))
	if err == nil && len(userAlreadyExists) > 0 {
		return repository.User{}, util.NewHTTPError(http.StatusConflict, "user with this email already exists")
	}
	...
	if err != nil {
		return repository.User{}, util.NewHTTPError(http.StatusInternalServerError, "failed to create user")
	}
	...
	if err != nil || len(created) == 0 {
		return repository.User{}, util.NewHTTPError(http.StatusInternalServerError, fmt.Sprintf("failed to fetch created user: %v", err))
	}
